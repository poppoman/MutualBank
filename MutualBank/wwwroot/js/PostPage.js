...
//新增留言
$(".messageboard-frame").on("click", "button:eq(0)", function () {
...
//新增回覆留言
$(".messageboard-frame").on("click", "button:eq(2)", function () {

    var reextareaVal = $("textarea:eq(1)").val()
    var fapadcontent = $("button:eq(2)").parents(".pad-content")
...
$(".messageboard-frame").on("click", "[class=replay-btn]", function (e) {
    ...
    if ($(".textarea-box").length == 1 & a.length >= 1) {
        ...
        $(".textarea-box:eq(1)").append("<textarea type='text' class='form-control' id='exampleFormControlTextarea1' placeholder='請輸入你的訊息' maxlength='200' rows='3'></textarea><div class='statics-text'>0/200字</div>")
        $(".col-auto-input").append(" <button type='button' class='btn btn-primary' disabled data-bs-toggle='button' id='1'>留言</button>")
    }
